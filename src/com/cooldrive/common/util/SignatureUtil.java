package com.cooldrive.common.util;
...
public class SignatureUtil {
	 ...
	 * @return
	 */
	public static String createSignForSortedMap(SortedMap<String, String> map, String apiKey, String characterEncoding) {
		String result = notSignParamsForSortedMap(map, apiKey);
		 logger.info("Sign Before MD5:" + result);
		result = MD5Util.MD5Encode(result, characterEncoding).toUpperCase();
		logger.info("Sign Result:" + result);
		return result;
	 ...
	 * @return 待加密的字符
	 */
	public static String notSignParamsForSortedMap(SortedMap<String, String> params, String apiKey) {
	...
	public static boolean checkIsSignValidFromWeiXin(String checktXml) throws ParserConfigurationException, IOException, SAXException, DocumentException {
		...
		logger.info("服务器回包里面的签名是:" + signFromresultXml);
		// 清掉返回数据对象里面的Sign数据（不能把这个数据也加进去进行签名），然后用签名算法进行签名
		map.put("sign", "");
		// 将API返回的数据根据用签名算法进行计算新的签名，用来跟API返回的签名进行比较
